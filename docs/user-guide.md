...
V3.9.0 合并 PR #84（贡献者 @Zanetach）的 profile env/status routing 基础，并将诊断和恢复能力收束为可选运维卡；这不是普通会话卡的视觉或 footer 更新。
...
- **profile 路由排障**：setup 的 `--profile-id` / `--event-url` 显式参数优先于进程环境、选定 env file 和默认值；`status`、`doctor`、`/health` 输出脱敏 route chain/profile diagnostics，用于定位 profile 或 endpoint mismatch。
...
| 版本 | 日期 | 主要变更 |
|------|------|---------|
| [v3.9.0](https://github.com/baileyh8/hermes-feishu-streaming-card/releases/tag/v3.9.0) | 2026-07 | PR #84 / @Zanetach：profile env/status route-chain、运维安全修复/重启与 CLI fallback；普通 footer/layout 不变 |
...
- [Zanetach](https://github.com/Zanetach) — [PR #84](https://github.com/baileyh8/hermes-feishu-streaming-card/pull/84) profile 环境变量与 `status` route-chain routing 基础（V3.9.0）
