...
V4.6.6 增加 15 秒到期与同 scope 确认投递提前唤醒；使用已有创建时间恢复计时，不重置重复登记期限。原生 requester、drain、后台成功一行、过期审批一行有独立 producer 证明；普通短提示仍为进程内计时，完整回执、失败和输出不撤回。详见[生命周期](notice-lifecycle.md)。
