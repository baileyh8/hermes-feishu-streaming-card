...
- Real Feishu private-chat baseline passed on 2026-07-11: `/hfc doctor` without a gray native unknown-command reply, localized details, two consecutive rechecks including the background successor, same-card updates, and the normal streaming-card footer.
- Pending real Feishu acceptance: repair/restart, group initiator repair/restart, changed-operator rejection, topic, cron, and profile route mismatch. These are not claimed as verified here.
