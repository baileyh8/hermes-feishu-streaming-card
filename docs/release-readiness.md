...
当前发布候选为 `4.6.8`。CardKit 实体重启恢复（从消息体 card 引用重建 entity，仅 CardKit 卡触发）、230011 撤回消息死循环终止、Hermes 0.21.x patcher 容忍。完整回归、精确提交 CI 和公开安装验证仍是发布门禁。

报告者 Hermes `8017dfa4a8` 的源码升级、重复安装、真实回调执行与逐字还原已验证；不宣称生产升级或手机真机验收。恢复不包含旧审批、执行栈或升级前无检查点的历史卡片。
