...
当前发布版本为 `4.6.8`。范围：macOS 安装与恢复提示、未知进程归属及自主管理登录启动说明。验收针对 CLI 平台分支和普通安装包；未新增 launchd 管理或卡片行为。

全量测试、精确合并 CI、资产校验和公开 tag 普通安装仍是发布门禁。[V4.6.8 发布说明](release-notes-v4.6.8.md) 列明本版验收范围；生产上线结果独立登记，不由提示测试推断真实 launchd 操作或手机验收通过。历史卡片证据保留在 [V4.6.6 验收清单](wiki/feishu-acceptance-v4.6.6.md)。
