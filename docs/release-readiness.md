...
当前包版本为 `0.1.0`。这一版定位为 sidecar-only 主线的首个可审阅版本，适合通过 PR 继续做真实环境 smoke 和小范围试用。
...
- Feishu CardKit HTTP client，已用 mock Feishu server 覆盖 tenant token、发送和更新。
...
- 真实 Hermes Gateway 进程 smoke test。
- 真实飞书应用 CardKit smoke test。

真实飞书联调只能使用本机配置或环境变量提供 `FEISHU_APP_ID` 和 `FEISHU_APP_SECRET`。不要把 App Secret、tenant token、真实 chat_id 或真实聊天截图提交到仓库。

## 当前边界

自动化测试不会访问真实飞书，也不会启动真实 Hermes Gateway。`smoke-feishu-card` 是人工联调工具，成功后应记录脱敏结果，而不是提交凭据或敏感截图。
